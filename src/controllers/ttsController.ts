 ...
 */
export const generateTTSSegments: RequestHandler = async (req, res, next) => {
 ...
 * 生成语音（单段）
 */
export const generateTTS: RequestHandler = async (req, res, next) => {
  try {
    const { text, voice } = req.body;
    ...
    try {
      ...
      if (serviceError.message && serviceError.message.includes('DASHSCOPE_API_KEY is not set')) {
        res.status(500).json({
          error: "DASHSCOPE_API_KEY is not set"
        });
        return;
